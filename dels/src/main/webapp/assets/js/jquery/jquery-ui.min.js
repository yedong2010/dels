...
margin:0,padding:0}),d={width:a.width(),height:a.height()},f=document.activeElement;try{f.id}catch(g){f=document.body}a.wrap(c);(a[0]===f||b.contains(a[0],f))&&b(f).focus();c=a.parent();"static"===a.css("position")?(c.css({position:"relative"}),a.css({position:"relative"})):(b.extend(e,{position:a.css("position"),zIndex:a.css("z-index.html")}),b.each(["top","left","bottom","right"],function(b,c){e[c]=a.css(c);isNaN(parseInt(e[c],10))&&(e[c]="auto")}),a.css({position:"relative",top:0,left:0,right:"auto",
...
this.handles=a.add(b(e.join("")).appendTo(this.element));this.handle=this.handles.eq(0);this.handles.each(function(a){b(this).data("ui-slider-handle-index.html",a)})},_createRange:function(){var d=this.options,f="";d.range?(!0===d.range&&(d.values?d.values.length&&2!==d.values.length?d.values=[d.values[0],d.values[0]]:b.isArray(d.values)&&(d.values=d.values.slice(0)):d.values=[this._valueMin(),this._valueMin()]),this.range&&this.range.length?this.range.removeClass("ui-slider-range-min ui-slider-range-max").css({left:"",
...
a,e,c=b(d.target).data("ui-slider-handle-index.html");switch(d.keyCode){case b.ui.keyCode.HOME:case b.ui.keyCode.END:case b.ui.keyCode.PAGE_UP:case b.ui.keyCode.PAGE_DOWN:case b.ui.keyCode.UP:case b.ui.keyCode.RIGHT:case b.ui.keyCode.DOWN:case b.ui.keyCode.LEFT:if(d.preventDefault(),!this._keySliding&&(this._keySliding=!0,b(d.target).addClass("ui-state-active"),f=this._start(d,c),!1===f))return}e=this.options.step;f=this.options.values&&this.options.values.length?a=this.values(c):a=this.value();switch(d.keyCode){case b.ui.keyCode.HOME:a=
...
keyup:function(d){var f=b(d.target).data("ui-slider-handle-index.html");this._keySliding&&(this._keySliding=!1,this._stop(d,f),this._change(d,f),b(d.target).removeClass("ui-state-active"))}}})})(jQuery);
