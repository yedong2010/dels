'use strict';
/**
 * @description 数据分析-业务能力分析
 * @author m13624
 * @date 2017-03-15
 */
app.controller('analysisBusinessCtrl', ['$scope', '$state', 'commonService', '$sce', '$stateParams', function ($scope, $state, commonService, $sce, $stateParams) {
    /**
    ...
    $scope.queryData = function () {
        commonService.queryDataByP('analysis/analysisbusiness',{
            startTime: $scope.startTime,
            endTime: $scope.endTime
        }).then(function (data) {
            for(var i = 0; i<data.length; i++){
                data[i].ywsl = parseInt(data[i].ywsl);
                data[i].ywbj = parseInt(data[i].ywbj);
                data[i].wywsx = parseInt(data[i].wywsx);
                data[i].fwjcl = Math.round( parseFloat(data[i].fwjcl) * 100 )/100;
            }
            vm.items = data;
        }, function () {
