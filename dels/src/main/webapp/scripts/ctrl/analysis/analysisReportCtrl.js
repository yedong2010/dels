'use strict';
/**
 * @description 数据分析-投诉举报分析
 * @author m13624
 * @date 2017-03-14
 */
app.controller('analysisReportCtrl', ['$scope', '$state', 'commonService', '$sce', '$stateParams', function ($scope, $state, commonService, $sce, $stateParams) {
    /**
    ...
    $scope.queryData = function () {
        commonService.queryDataByP('analysis/analysisreport',{
            startTime: $scope.startTime,
            endTime: $scope.endTime
        }).then(function (data) {
            for(var i = 0; i<data.length; i++){
                data[i].tsjb = Math.round( parseFloat(data[i].tsjb) * 100 )/100;
                data[i].myd = Math.round( parseFloat(data[i].myd) * 100 )/100;
                data[i].ywl = parseInt(data[i].ywl);
                data[i].ywblnl = Math.round( parseFloat(data[i].ywblnl) * 100 )/100;
                data[i].rkcd = Math.round( parseFloat(data[i].rkcd) * 100 )/100;
            }
            vm.items = data;
        }, function () {
