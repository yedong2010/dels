...
/**
 * @ngdoc controller
 * @name app.controller.allBusinessCtrl
 * @description 数据质量监测-进驻事项数据版本-全省
 * @author m13624
 * @date 2017-01-05
 */
app.controller('allBusinessCtrl', ['$scope', '$state', 'commonService', '$sce', '$stateParams', function ($scope, $state, commonService, $sce, $stateParams) {
    var vm = $scope.vm = {};
    vm.page = {
        size: 5,
        index: 1
     ...
     * @description 进驻事项数据版本，全省和其他（地市、省级、区县）不是同一个页面，所以根据用户类型判断进入的类型，判断进去哪个页面
     */
    var userRolName = commonService.getRolName();
    $scope.type = commonService.getUserCityType();

    if(userRolName == '分析师'){
        $state.go('quality.business.other', {place: $scope.type});
    }
    ...
    /**
     * @description 默认版本一是昨日事项数，版本二十今日事项数
     */
    ...
    $scope.queryAllBusStartTime = function (choseDate) {
        ...
        commonService.queryDataByP('quality/qsjzsxs/' + choseDate).then(function (data) {
    ...
    $scope.queryAllBusEndTime(today);
    ...
    $scope.queryBusCompare = function(){
        ...
        commonService.queryDataByP('quality/qsjzsxsdb/' + $scope.startTime + '/' + $scope.endTime).then(function (data) {
