...
/**
 * @ngdoc controller
 * @name app.controller.applyRankCtrl
 * @description 数据指标统计-申办业务量排名
 * @author m13624
 * @date 2017-03-27
 */
app.controller('applyRankCtrl', ['$scope', '$state','$stateParams', '$timeout', 'commonService', '$templateCache', function ($scope, $state, $stateParams, $timeout, commonService, $templateCache) {
    /**
    ...
    $scope.queryByParam = function () {
        ...
        vm.items = null;
        /**
         * @description 查询当前类型下的申办业务量排名（全省显示事项排名）
         ...
         */
        commonService.queryDataByP('totality/sbywlph/' + $scope.type,  {
    ...
    $scope.goDetail = function (obj) {
        ...
        $state.go('indicators.applyRankDetail', {
            type: $scope.type,
            keyId: $scope.keyId,
            startTime: $scope.startTime,
            endTime: $scope.endTime,
