...
/**
 * @ngdoc controller
 * @name app.controller.busRankDetailCtrl
 * @description 数据指标统计-办结业务量排名-详情页
 * @author m13624
 * @date 2017-03-24
 */
app.controller('busRankDetailCtrl', ['$scope', '$state', '$timeout', '$stateParams', 'commonService', '$templateCache', function ($scope, $state, $timeout, $stateParams, commonService, $templateCache) {
    /**
    ...
    $scope.queryBusDetail = function () {
         ...
         */
        vm.items = null;
        commonService.queryDataByP('totality/bjywlphDetail/' + $scope.type, {
            id: $scope.keyId,
    ...
    $scope.goParent = function () {
        $state.go('indicators.busRank', {type: $scope.type});
    };
}]);